...
def job_detail_technician(request, job_code):
    ...
    if request.method == 'POST':
        ...
        if action == 'update_status':
            ...
            if request.headers.get('x-requested-with') == 'XMLHttpRequest':
                # If the job moved to Ready for Pickup, tell the client to
                # redirect to the dashboard instead of staying on this page.
                redirect_url = (
            ...
            if job.status == 'Ready for Pickup':
                messages.success(request, f'Job {job.job_code} marked as completed and moved to Ready for Pickup.')
